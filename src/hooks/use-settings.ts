...
export function useSettings(userId: string = 'current-user') {
  ...
  // Helper function to apply all preferences to UI
  const applyPreferencesToUI = (prefs: UserPreferences) => {
    // Apply card style
    if (prefs.cardStyle) {
      document.body.classList.remove('card-style-default', 'card-style-flat', 'card-style-glass');
      document.body.classList.add(`card-style-${prefs.cardStyle}`);
    }
    ...
    if (prefs.customColors) {
      document.documentElement.style.setProperty('--color-primary', prefs.customColors.primary);
      document.documentElement.style.setProperty('--color-secondary', prefs.customColors.secondary);
      document.documentElement.style.setProperty('--color-accent', prefs.customColors.accent);
      document.documentElement.style.setProperty('--color-background', prefs.customColors.background);
      document.documentElement.style.setProperty('--color-text', prefs.customColors.text);
      document.documentElement.style.setProperty('--color-border', prefs.customColors.border);
    }
    ...
    if (prefs.customBackground) {
      ...
      if (prefs.customBackground.type === 'pattern') {
        ...
        const selectedPattern = patterns.find(p => p.id === prefs.customBackground.value);
        if (selectedPattern) {
        ...
        document.body.style.backgroundImage = `url(${prefs.customBackground.value})`;
      }
    } else {
  ...
  // Helper function to apply a specific preference to UI
  const applySpecificPreferenceToUI = (key: keyof UserPreferences, value: any, prefs: UserPreferences) => {
    switch (key) {
      case 'cardStyle':
        document.body.classList.remove('card-style-default', 'card-style-flat', 'card-style-glass');
      ...
      case 'customColors':
        if (value) {
          ...
          document.documentElement.style.setProperty('--color-accent', value.accent);
          document.documentElement.style.setProperty('--color-background', value.background);
          document.documentElement.style.setProperty('--color-text', value.text);
          document.documentElement.style.setProperty('--color-border', value.border);
        } else {
      ...
      case 'customBackground':
        if (value) {
          ...
          if (value.type === 'pattern') {
            ...
            const selectedPattern = patterns.find(p => p.id === value.value);
            if (selectedPattern) {
              document.body.style.backgroundImage = `url(${selectedPattern.url})`;
            }
          } else {
            ...
            document.body.classList.add('background-image');
            document.body.classList.remove('background-pattern');
            document.body.style.backgroundImage = `url(${value.value})`;
          }
        } else {
          // No custom background
          document.body.classList.remove('custom-background', 'background-pattern', 'background-image');
