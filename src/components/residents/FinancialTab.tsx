...
import { Download, ArrowUp, ArrowDown, DollarSign, Calendar, CreditCard } from 'lucide-react';
import { Card, CardHeader, CardTitle, CardDescription, CardContent } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/components/ui/table';
import { Tabs, TabsList, TabsTrigger, TabsContent } from '@/components/ui/tabs';
import { Transaction, ResidentProfile } from '@/types/resident';
...
const FinancialTab: React.FC<FinancialTabProps> = ({ transactions = [], resident }) => {
  ...
  // Check if resident has a balance
  const hasBalance = () => {
    if (!transactions || transactions.length === 0) return false;
    const balanceStr = transactions[0].balance.replace('$', '').replace(',', '');
